...
__global__ void kernel(double step, double * GPU) {
    __shared__ double sumGPU[nbThreadPerBlock];
    int tid = threadIdx.x + blockIdx.x * blockDim.x, i;
    double result = 0.0, x, final = 0.0;
    ...
    sumGPU[threadIdx.x] = result;
    
    __syncthreads();
    
    for (int i = 0; i < nbThreadPerBlock; i++) {
        final += sumGPU[i];
    }
    
    // Affectation du resultat d'un seul thread
    GPU[blockIdx.x] = final;
...
int main ()
{
    ...
    sum = (double*) malloc(nbBlocks * sizeof(double));

    step = 1.0/(double) num_steps;
    
    start_time = omp_get_wtime();
    
    HANDLE_ERROR(cudaMalloc((void**)&dev_Sum, sizeof(double) * (nbBlocks)));
    kernel<<<nbBlocks, nbThreadPerBlock>>>(step, dev_Sum);
    HANDLE_ERROR(cudaMemcpy(sum, dev_Sum, sizeof(double) * nbBlocks, cudaMemcpyDeviceToHost));
